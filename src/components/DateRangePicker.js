...
const presets = [
  { label: 'Past Week', value: 'past-week', days: 7 },
  { label: 'Past Month', value: 'past-month', months: 1 },
  { label: 'Past 3 Months', value: 'past-3-months', months: 3 },
  { label: 'Past 6 Months', value: 'past-6-months', months: 6 },
  { label: 'Past Year', value: 'past-year', years: 1 },
...
const DateRangePicker = ({ onSelectRange, onClose, triggerRef }) => {
  ...
  const handlePreset = (preset) => {
    ...
    onSelectRange({ start, end: preset.value !== 'clear' ? now : null, label: preset.label });
  };
  ...
  const renderCalendar = (offset = 0) => {
    ...
    return (
      <div className="w-64 p-2">
        <h4 className="text-sm font-medium text-gray-700 text-center mb-2">{monthLabel}</h4>
        ...
        <div className="grid grid-cols-7 gap-1 text-sm">
          {days.map((day, i) => {
            ...
            const isStart = startDate && day && day.toDateString() === startDate.toDateString();
            const isEnd = endDate && day && day.toDateString() === endDate.toDateString();

            return (
              <div
                key={i}
                className={`h-8 w-8 flex items-center justify-center rounded cursor-pointer
                  ${!day ? '' : isSelected ? 'bg-blue-100 text-blue-700' : 'hover:bg-gray-100'}
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div ref={pickerRef} className="bg-white rounded-lg p-4 shadow-xl flex space-x-4">
        {/* Sidebar presets */}
        ...
        <div className="flex flex-col space-y-4">
          ...
          <div className="flex justify-end">
            <button
              ...
              onClick={() => onSelectRange({ start: startDate, end: endDate, label: 'Custom Range' })}
